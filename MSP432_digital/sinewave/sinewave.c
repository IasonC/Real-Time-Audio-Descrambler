 ...
 *     Purpose: To test the sinewave produced by the code
 */
...
// Declare sinewave parameters:
#define PI 3.14159265359
/*static volatile float sine_2[50];
static volatile float sine_5[50];
static volatile float cosine_2[50];
static volatile float cosine_5[50];
static volatile float sin7k[50];*/

volatile int sin_value[PTS];

//#include <rom_map.h> // this is necessary to use MAP prefix for funcs
    // MAP is a macro that decides to use ROM or Flash version of each function
...
int main(void)
{
    ...
    /*int s2 = 0; int s5 = 0; int c2 = 0; int c5 = 0; int k = 0;
    for (s2 = 0; s2 < 50; s2++){
        float freq = 2*PI*2.0f;
        float arg = s2 / 50.0f;
        sine_2[s2] = cos(freq * arg);
    }
    for (s5 = 0; s5 < 50; s5++){
        float freq = 2*PI*5000;
        float arg = s5 / 50000;
        sine_5[s5] = sin(freq * arg);
    }
    for (c2 = 0; c2 < 50; c2++){
        float freq = 2*PI*2000;
        float arg = c2 / 50000;
        cosine_2[c2] = cos(freq * arg);
    }
    for (c5 = 0; c5 < 50; c5++){
        float freq = 2*PI*2000;
        float arg = c5 / 50000;
        cosine_5[c5] = cos(freq * arg);
    }
    for (k = 0; k < 50; k++){
        sin7k[k] = sine_2[s2] * cosine_5[c5] + sine_5[s5] * cosine_2[c2];
    ...
    for( t = 0; t < PTS; t = t + 1)
    {
        sin_value[t] = 255/2 * ( ( sin(2*PI*2*(t/(float)PTS))*cos(2*PI*5*(t/(float)PTS)) +
...
int sinecount = 0; // counter for SysTick
...
void SysTick_Handler(void)
{
    ...
    /*P2OUT = sin7k[sinecount];
    sinecount++;
    if (sinecount == 50) {sinecount = 0;} // loop back for the next period of sinewave*/
